...
export class RegisterInWhitelist {
  constructor(private readonly userRepository: UserRepository) {}

  public async execute(newUserEmail: string): Promise<void> {
    try {
      ...
      const existingUsers = await this.userRepository.listAll();
      const existingEmails = existingUsers.map((user) => user.getEmail().getValue());

      // Agregar el nuevo email a la lista si no existe ya
      const allEmails = existingEmails.includes(newUserEmail) ? existingEmails : [...existingEmails, newUserEmail];
